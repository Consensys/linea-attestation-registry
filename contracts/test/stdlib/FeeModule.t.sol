...
import { FeeModule, AbstractModule } from "../../src/stdlib/FeeModule.sol";
import { AttestationPayload } from "../../src/types/Structs.sol";
import { PortalRegistryMock } from "../mocks/PortalRegistryMock.sol";

contract FeeModuleTest is Test {
  FeeModule private feeModule;
  ...
  function setUp() public {
    ...
    feeModule = new FeeModule(address(portalRegistry));
  ...
  function test_setFees_ArrayLengthMismatch() public {
    vm.expectRevert(FeeModule.ArrayLengthMismatch.selector);
  ...
  function test_run_TwoPortals() public {
    ...
    feeModule.run(attestationPayload1, new bytes(0), user1, attestationFees[0]);
    ...
    vm.expectRevert(FeeModule.InvalidAttestationFee.selector);
    feeModule.run(attestationPayload2, new bytes(0), user1, 0);
  }
  ...
  function test_run_InvalidAttestationFee() public {
    ...
    vm.expectRevert(FeeModule.InvalidAttestationFee.selector);
    vm.startPrank(portal);
    feeModule.run(payload, new bytes(0), user1, 0);
  }
}
